...
namespace Memory1
{
    public partial class Memory : Form
    {
        ...
        //controlla se una carta è già stato generata più di due volte
        private int controlloCarte(string[] arrayCard, ref string animationGenerato, ref int nGentroll, ref int nGensax, ref int nGenpirate, ref int nGensuperman)//controlla se una carta è già stato generata più di due volte
        {
            
            arrayCard = arrayCard.Where(c => c != null).ToArray();//elimina i valori nulli
        ...
        private int aumentoNumAnimGen(int genNum)//controlla se ogni variabile è stata sorteggiata più di due volte
        {
            if (genNum < 3)
            {
                genNum++;
        ...
        private System.Drawing.Image immaginecarta(int nCarta)//assegna alle carte un immagine random
        {
        ...
        private void clickCarta(int numCarta)//funzione per il clikc di una carta
        {
        ...
        private void controlloCoppie(ref int nCard)
        {
            if (carteSelezionate % 2 == 0)
            {
                ...
                if (animationCarte[card1] != animationCarte[card2])
                {
                    ...
                    wait(500);//aspetta 0,5s per procedere
                    assegnazioneSfondoCarteNonCoppie(card1);
                    assegnazioneSfondoCarteNonCoppie(card2);
                    carteSelezionate -= 2;
                    turno = !turno;//cambia il valore da true a false e viceversa
                    if (turno)//cambia il colore dello sfondo in base al turno
                    {
                        this.BackColor = Color.Beige;
                        nturno.Text = player2;
                    }
                ...
                {
                    if (turno)//assegna il punteggio in base del turno
                    {
                        coppie2++;
                    }
                    else
            ...
            if (carteSelezionate == 8)//tasto per ricominciare il gioco
            {
                restartGame.Visible = true;
                this.BackColor = Color.White;
                if (coppie1 > coppie2)//mostra chi è il vincitore
                {
        ...
        public void wait(int milliseconds)
        {
            ...
            //controlla che il valore dei millisecondi sia >=0
        ...
        private void restartGame_Click(object sender, EventArgs e)//funzione per riiniziare il gioco
        {
            ...
            else
            {
                ...
                scrittaWannaPlay.Visible = false;
                scrittaInserisciNickname.Visible = false;
            }
